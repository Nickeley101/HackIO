function myLoop(myMouseEvent) {         //  create a loop function
    ...
    setTimeout(function () {   //  call a 200ms setTimeout when the loop is called
        ...
        let loss = false;
        // if (hopeless && tankRet[1].length !== 0) { //guess on lowest probability row
        ...
        //     loss = checkLoss(face, myMouseEvent);
        ...
        let flagsRemaining = document.getElementById("mines_hundreds").className[document.getElementById("mines_hundreds").className.length - 1];
        flagsRemaining += document.getElementById("mines_tens").className[document.getElementById("mines_tens").className.length - 1];
        flagsRemaining += document.getElementById("mines_ones").className[document.getElementById("mines_ones").className.length - 1];
        // || flagsRemaining === "001"
        if ((flagsRemaining === "000") && !loss && !hopeless && cycle) { //if no/one flag(s) are left and you aren't dead, click blank squares
            console.log("tricky tricky");
            // for (let i = 1; i <= dims[0] && cycle; i++) { //iterate through each square and click blanks
            //     for (let j = 1; j <= dims[1] && cycle; j++) {
            //         let squareID = document.getElementById(i + "_" + j);

            //         if (squareID.className === "square blank") {
...
function checkLoss(face, myMouseEvent) {
    let loss = false;
...
function simulateClick() {
    ...
    myMouseEvent[4] = new KeyboardEvent('keydown', {
        code: 'Enter',
        key: 'Enter',
        charCode: 13,
        keyCode: 13,
